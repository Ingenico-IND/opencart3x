...
class ControllerExtensionPaymentWorldline extends Controller {
    public function index() {
        ...
        $returnUrl_2 = $this->url->link('extension/payment/Worldline/getResponse');
    ...
    public function getResponse() {
        if($_POST){
            $salt = $this->config->get('payment_Worldline_key');
            $str = $_POST['msg'];
           //print_r($str); die;
            $log = new Log('Worldline_' . date("Ymd") . '.log');
            ...
            if ($hashedString == $verificationHash) {
                ...
                $data_array = array(
                    "merchant" => array(
                        "identifier" => $this->config->get('payment_Worldline_merchant_code')
                    ),
                    "transaction" => array(
                        "deviceIdentifier" => "S",
